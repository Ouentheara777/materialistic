...
public class ListFragmentTest {
    ...
    public void testHighlightNewItems() {
        when(itemManager.getStories(any(), eq(ItemManager.MODE_DEFAULT)))
                .thenReturn(new Item[]{new TestItem() {
                    @Override
                    public String getId() {
                        return "1";
                    }
        ...
        reset(itemManager);
        when(itemManager.getStories(any(String.class), eq(ItemManager.MODE_NETWORK)))
                .thenReturn(new Item[]{ new TestHnItem(1L), new TestHnItem(2L) });
        ShadowSwipeRefreshLayout shadowSwipeRefreshLayout = (ShadowSwipeRefreshLayout)
                ShadowExtractor.extract(activity.findViewById(R.id.swipe_layout));
        shadowSwipeRefreshLayout.getOnRefreshListener().onRefresh();
        // should trigger another data request
